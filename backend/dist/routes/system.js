    ...
    try {
        ...
        const status = {
            ...
            gpu: {
                ...
                name: gpuStatus.name,
                memory_total: gpuStatus.available ? parseFloat(gpuStatus.memory_total.replace('GB', '')) * 1024 : 0,
                memory_used: gpuStatus.available ? parseFloat(gpuStatus.memory_used.replace('GB', '')) * 1024 : 0,
...
async function checkModelsLoaded() {
    try {
        try {
            const aiResponse = await axios_1.default.get(`${AI_SERVICE_URL}/health`, { timeout: 3000 });
        ...
        for (const [key, model] of Object.entries(models)) {
            if (typeof model === 'object' && model !== null && 'path' in model && 'enabled' in model) {
                ...
                if (modelData.enabled && await fs_extra_1.default.pathExists(modelData.path)) {
                    loadedCount++;
                }
            }
        }
        return totalCount > 0 && loadedCount > 0;
    }
    catch (error) {
