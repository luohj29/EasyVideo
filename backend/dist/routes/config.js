...
const DEFAULT_CONFIG_PATH = path_1.default.join(__dirname, '../../../config/default.json');
router.get('/', async (req, res) => {
    try {
        let config;
        if (await fs_extra_1.default.pathExists(CONFIG_PATH)) {
            config = await fs_extra_1.default.readJson(CONFIG_PATH);
        }
        else {
...
router.post('/', async (req, res) => {
    try {
        ...
        await fs_extra_1.default.ensureDir(path_1.default.dirname(CONFIG_PATH));
        await fs_extra_1.default.writeJson(CONFIG_PATH, newConfig, { spaces: 2 });
...
router.post('/reset', async (req, res) => {
    try {
        ...
        await fs_extra_1.default.ensureDir(path_1.default.dirname(CONFIG_PATH));
        await fs_extra_1.default.writeJson(CONFIG_PATH, defaultConfig, { spaces: 2 });
...
async function validateModelPaths(config) {
    const results = {
        valid: true,
        models: {},
        paths: {}
    ...
    if (config.models) {
        for (const [key, model] of Object.entries(config.models)) {
            if (typeof model === 'object' && model !== null && 'path' in model) {
                ...
                const exists = await fs_extra_1.default.pathExists(modelData.path);
                results.models[key] = {
                    path: modelData.path,
                    exists,
                    accessible: exists
    ...
    if (config.paths) {
        for (const [key, pathValue] of Object.entries(config.paths)) {
            if (typeof pathValue === 'string') {
                const exists = await fs_extra_1.default.pathExists(pathValue);
                results.paths[key] = {
                    path: pathValue,
                    exists,
                    accessible: exists
                };
                if (!exists && key !== 'temp_dir')
                    results.valid = false;
            }
        }
    }
...
router.post('/import', async (req, res) => {
    try {
        ...
        await fs_extra_1.default.ensureDir(path_1.default.dirname(CONFIG_PATH));
        await fs_extra_1.default.writeJson(CONFIG_PATH, importedConfig, { spaces: 2 });
...
router.post('/check-path', async (req, res) => {
    try {
        ...
        if (exists) {
            try {
                const stats = await fs_extra_1.default.stat(checkPath);
                size = stats.size;
            }
            catch (err) {
